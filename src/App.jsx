...
const glass = 'bg-white/[0.13] border border-white/[0.26] backdrop-blur-2xl rounded-3xl p-6 shadow-[0_32px_130px_rgba(0,0,0,0.18)]'

const tabs = [
  { id: 'home', label: 'Home', icon: Home },
  { id: 'licao', label: 'Aula', icon: Layers3 },
]
...
export default function App() {
  ...
  const buttonPreviewClass = useMemo(() => {
    ...
    if (states.hover) classes.push('shadow-[inset_0_1px_0_rgba(255,255,255,0.9),inset_0_-18px_34px_rgba(0,0,0,0.08),0_0_80px_rgba(20,184,166,0.48)] ring-1 ring-cyan-100/70 -translate-y-1')
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#eafff4] text-[#071011]">
      <CinematicBackground glowY={glowY} glowScale={glowScale} />
      <AnimatePresence>{!loaded && <LoadingState />}</AnimatePresence>
...
function CinematicBackground({ glowY, glowScale }) {
  return <><motion.div style={{ y: glowY, scale: glowScale }} className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_10%_0%,rgba(45,212,191,0.58),transparent_34%),radial-gradient(circle_at_100%_8%,rgba(168,85,247,0.34),transparent_27%),radial-gradient(circle_at_52%_45%,rgba(255,255,255,0.82),transparent_36%),linear-gradient(180deg,#fffefa_0%,#c8f8e5_30%,#f4fff7_55%,#070a0a_100%)]" /><div className="pointer-events-none absolute inset-0 opacity-[0.075] mix-blend-multiply bg-[repeating-linear-gradient(0deg,#000_0px,#000_1px,transparent_1px,transparent_4px)]" /><div className="pointer-events-none absolute left-[-90px] top-[-110px] h-96 w-96 rounded-full bg-cyan-300/40 blur-[140px]" /><div className="pointer-events-none absolute bottom-32 right-[-120px] h-96 w-96 rounded-full bg-purple-500/35 blur-[140px]" /><Particles /><motion.div animate={{ y: [0, -16, 0], rotate: [0, 4, 0] }} transition={{ duration: 9, repeat: Infinity }} className="pointer-events-none absolute right-4 top-32 h-28 w-28 rounded-[2rem] border border-white/35 bg-white/20 backdrop-blur-2xl" /><motion.div animate={{ y: [0, 20, 0], rotate: [0, -6, 0] }} transition={{ duration: 11, repeat: Infinity }} className="pointer-events-none absolute left-5 top-[38rem] h-20 w-20 rounded-full border border-cyan-200/40 bg-cyan-200/20 blur-[1px]" /></>
}

function Particles() {
  return <div className="pointer-events-none absolute inset-0 overflow-hidden">{Array.from({ length: 18 }).map((_, index) => <motion.span key={index} animate={{ y: [0, -18 - index, 0], opacity: [0.18, 0.55, 0.18] }} transition={{ duration: 4 + index * 0.2, repeat: Infinity, delay: index * 0.17 }} className="absolute h-1.5 w-1.5 rounded-full bg-white/70 shadow-[0_0_18px_rgba(255,255,255,0.8)]" style={{ left: `${(index * 37) % 100}%`, top: `${12 + ((index * 19) % 78)}%` }} />)}</div>
}

function LoadingState() {
  return <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.45 }} className="fixed inset-0 z-[100] grid place-items-center bg-[#eafff4]"><motion.div initial={{ scale: 0.86, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 1.06, opacity: 0 }} transition={spring} className="relative h-32 w-32 rounded-[2rem] border border-white/50 bg-white/30 shadow-[0_30px_120px_rgba(0,0,0,0.18)] backdrop-blur-2xl"><motion.div animate={{ x: [12, 72, 12], y: [18, 70, 18], borderRadius: ['50%', '34%', '50%'] }} transition={{ duration: 1.8, repeat: Infinity }} className="absolute h-10 w-10 bg-cyan-300 shadow-[0_0_50px_rgba(34,211,238,0.75)]" /><div className="absolute bottom-5 left-0 right-0 text-center text-xs font-black uppercase tracking-[0.24em] text-black/45">Loading</div></motion.div></motion.div>
}
...
function HomeScreen({ xp, vidas, streak, openLesson, promptAtivo, setPromptAtivo }) {
  return <div className="space-y-5"><HeroCard openLesson={openLesson} /><div className="grid grid-cols-3 gap-3"><Metric label="Streak" value={streak} /><Metric label="XP" value={xp} /><Metric label="Vidas" value={vidas} /></div><InterfacePreview /><MiniVideoCard /><PromptMachine promptAtivo={promptAtivo} setPromptAtivo={setPromptAtivo} /><SectionTitle title="Trilha de hoje" subtitle="Aulas curtas com resultado visual" /> <div className="space-y-3">{lessons.map((lesson, index) => <motion.button key={lesson.title} whileTap={{ scale: 0.95 }} whileHover={{ y: -2 }} transition={spring} onClick={index === 0 ? openLesson : undefined} className={`${glass} flex w-full items-center justify-between text-left`}><div><div className="flex items-center gap-2"><span className="rounded-full border border-black/10 bg-white/45 px-2 py-1 text-[10px] font-black text-black/45">{lesson.tag}</span><span className="text-xs text-black/35">{lesson.time}</span></div><h3 className="mt-3 text-xl font-extrabold tracking-tighter">{lesson.title}</h3><p className="mt-1 text-sm text-black/42">{lesson.desc}</p></div><ChevronRight className="text-black/35" /></motion.button>)}</div><SectionTitle title="Renda digital" subtitle="Transforme aula em proposta" /><div className="grid gap-3">{projects.map((project) => <motion.article key={project.title} whileHover={{ y: -2 }} transition={spring} className={`${glass}`}><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.24em] text-black/35">Projeto</p><h3 className="mt-2 text-2xl font-extrabold tracking-tighter">{project.title}</h3><p className="mt-2 text-sm leading-6 text-black/45">{project.desc}</p></div><div className="rounded-2xl border border-emerald-600/10 bg-emerald-300/30 px-3 py-2 text-sm font-extrabold text-emerald-950">{project.value}</div></div></motion.article>)}</div><AchievementStrip /></div>
}

function HeroCard({ openLesson }) {
  return <motion.button whileTap={{ scale: 0.95 }} whileHover={{ y: -2, rotateX: 2, rotateY: -2 }} transition={spring} onClick={openLesson} className="relative w-full overflow-hidden rounded-[2.6rem] border border-white/50 bg-white/32 p-6 text-left shadow-[0_36px_140px_rgba(0,0,0,0.20)] backdrop-blur-2xl [transform-style:preserve-3d]"><div className="absolute -right-20 -top-20 h-56 w-56 rounded-full bg-cyan-300/40 blur-[80px]" /><motion.div animate={{ x: ['-30%', '130%'] }} transition={{ duration: 4.6, repeat: Infinity, ease: 'easeInOut' }} className="absolute inset-y-0 w-24 rotate-12 bg-white/25 blur-2xl" /><div className="absolute left-8 top-36 h-40 w-40 rounded-full bg-purple-400/20 blur-[70px]" /><div className="absolute bottom-0 left-0 right-0 h-52 bg-gradient-to-t from-black/70 via-black/18 to-transparent" /><div className="relative min-h-[470px]"><div className="mb-7 inline-flex items-center gap-2 rounded-full border border-white/50 bg-white/35 px-3 py-1.5 text-xs font-bold text-black/55 backdrop-blur-xl"><Crown size={13} /> Signature Series</div><h2 className="max-w-xs text-6xl font-extrabold leading-[0.82] tracking-tighter text-black">Botões que parecem produto caro.</h2><PremiumKeys /><motion.div animate={{ x: [0, 12, 0], y: [0, -8, 0] }} transition={{ duration: 4.8, repeat: Infinity }} className="absolute right-3 top-10 h-12 w-12 rounded-full bg-cyan-300/80 shadow-[0_0_60px_rgba(34,211,238,0.65)]" /><div className="absolute bottom-0 left-0 right-0 text-white"><p className="max-w-xs text-sm leading-7 text-white/72">Aprenda a criar CTA com presença, contraste, estados e toque mobile.</p><div className="mt-5 flex items-center gap-3"><span className="rounded-2xl bg-white px-5 py-3 text-sm font-extrabold text-black shadow-[0_18px_50px_rgba(255,255,255,0.24)]">Começar aula</span><span className="text-sm font-bold text-white/55">+15 XP</span></div></div></div></motion.button>
}

function PremiumKeys() {
  return <div className="absolute right-0 top-28 grid rotate-[-8deg] grid-cols-2 gap-2"><div className="h-20 w-20 rounded-[1.4rem] border border-white/50 bg-black/70 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),inset_0_-18px_34px_rgba(0,0,0,0.5),0_24px_60px_rgba(0,0,0,0.32)]" /><div className="h-20 w-20 rounded-[1.4rem] border border-orange-200/30 bg-orange-500 shadow-[inset_0_1px_0_rgba(255,255,255,0.28),inset_0_-20px_38px_rgba(120,0,0,0.45),0_24px_70px_rgba(249,115,22,0.32)]" /><div className="h-20 w-20 rounded-[1.4rem] border border-white/50 bg-black/65 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),inset_0_-18px_34px_rgba(0,0,0,0.5),0_24px_60px_rgba(0,0,0,0.32)]" /><div className="h-20 w-20 rounded-[1.4rem] border border-cyan-200/30 bg-cyan-500/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.28),inset_0_-20px_38px_rgba(0,50,70,0.45),0_24px_70px_rgba(34,211,238,0.32)]" /></div>
}
...
function MiniVideoCard() {
  return <motion.article whileHover={{ y: -2 }} transition={spring} className="relative overflow-hidden rounded-[2rem] border border-white/45 bg-black p-4 text-white shadow-[0_30px_110px_rgba(0,0,0,0.22)]"><motion.div animate={{ backgroundPosition: ['0% 0%', '100% 100%'] }} transition={{ duration: 5, repeat: Infinity, repeatType: 'reverse' }} className="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(34,211,238,0.25),transparent_24%),radial-gradient(circle_at_90%_20%,rgba(249,115,22,0.22),transparent_22%),linear-gradient(135deg,#050505,#132927,#0b0b0d)]" /><div className="relative min-h-[190px] rounded-[1.5rem] border border-white/10 p-5"><div className="flex items-center justify-between"><p className="text-xs uppercase tracking-[0.24em] text-white/45">Mini vídeo</p><div className="grid h-10 w-10 place-items-center rounded-full bg-white text-black"><Play size={16} fill="currentColor" /></div></div><motion.div animate={{ x: [0, 90, 0] }} transition={{ duration: 3.2, repeat: Infinity }} className="mt-10 h-16 w-28 rounded-3xl bg-white/12 backdrop-blur-xl" /><h3 className="mt-5 text-3xl font-extrabold tracking-tighter">Veja o antes e depois do CTA.</h3></div></motion.article>
}
