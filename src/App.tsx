...
export default function App() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white">
      ...
      <section className="relative mx-auto grid min-h-screen max-w-6xl gap-6 px-5 py-6 lg:grid-cols-[1fr_360px]">
        <div className="rounded-[2.5rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur-2xl">
          ...
          <p className="mt-3 text-sm text-zinc-500">Jogador: {player} • {alreadyVoted ? "voto registrado" : "você ainda não votou"}</p>
        ...
        <aside className="rounded-[2.5rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur-2xl">
          ...
          <button onClick={() => { localStorage.clear(); location.reload(); }} className="mt-3 w-full rounded-2xl border border-red-500/20 bg-red-500/10 px-5 py-4 text-sm font-black uppercase tracking-[0.16em] text-red-200">Limpar dados</button>
        </aside>
      </section>
    </main>
  );
