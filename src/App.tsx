...
export default function App() {
  ...
  function resetVotes() {
    ...
    const updated: Room = { ...currentRoom, options: currentRoom.options.map((opt) => ({ ...opt, votes: 0 })) };
    setCurrentRoom(updated);
    setRooms((current) => current.map((room) => room.id === updated.id ? updated : room));
    timer.reset();
  }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(168,85,247,0.32),transparent_32%),radial-gradient(circle_at_90%_18%,rgba(59,130,246,0.18),transparent_28%),radial-gradient(circle_at_50%_100%,rgba(236,72,153,0.12),transparent_30%)]" />
      <section className="relative mx-auto min-h-screen max-w-7xl px-5 py-5 md:px-8">
        <header className="flex items-center justify-between rounded-[1.7rem] border border-white/10 bg-white/[0.055] p-3 backdrop-blur-2xl">
          <button onClick={() => setScreen("home")} className="flex items-center gap-3 rounded-2xl px-2 py-2 text-left"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-lg font-black text-black">C</div><div><p className="text-sm font-black tracking-tight">ClashRoom</p><p className="text-xs font-bold text-zinc-500">Arena social de votos</p></div></button>
        ...
        {screen === "home" && <div className="grid gap-6 py-6 lg:grid-cols-[1fr_390px]"><section className="rounded-[2.8rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur-2xl md:p-9"><p className="text-xs font-black uppercase tracking-[0.24em] text-violet-200">Arena premium</p><h1 className="mt-4 max-w-4xl text-5xl font-black leading-[0.9] tracking-[-0.08em] md:text-7xl">Vote rápido. Vença no placar.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-400">ClashRoom agora está limpo por dentro e premium por fora: salas, duelos, ranking, timer e campeões.</p><div className="mt-9 grid gap-4 md:grid-cols-3"><div className="rounded-[2rem] border border-white/10 bg-black/35 p-5"><p className="text-sm text-zinc-500">Salas</p><p className="mt-3 text-4xl font-black">{rooms.length}</p></div><div className="rounded-[2rem] border border-white/10 bg-black/35 p-5"><p className="text-sm text-zinc-500">Votos</p><p className="mt-3 text-4xl font-black">{totalVotes}</p></div><div className="rounded-[2rem] border border-white/10 bg-black/35 p-5"><p className="text-sm text-zinc-500">Campeões</p><p className="mt-3 text-4xl font-black">{history.length}</p></div></div><div className="mt-9 flex flex-wrap gap-3"><button onClick={() => setScreen('create')} className="rounded-2xl bg-white px-6 py-4 text-sm font-black uppercase tracking-[0.16em] text-black">Criar sala</button><button onClick={() => setScreen('room')} className="rounded-2xl border border-white/10 bg-white/[0.06] px-6 py-4 text-sm font-black uppercase tracking-[0.16em] text-white">Entrar na arena</button></div></section><aside className="rounded-[2.8rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur-2xl"><p className="text-xs font-black uppercase tracking-[0.24em] text-zinc-500">Salas ativas</p><div className="mt-5 grid gap-3">{rooms.map((room) => <button key={room.id} onClick={() => selectRoom(room)} className="rounded-3xl border border-white/10 bg-black/35 p-4 text-left transition hover:bg-white/10"><p className="font-black">{room.title}</p><p className="mt-2 text-xs font-bold uppercase tracking-[0.14em] text-zinc-500">{room.category} • {room.mode === 'battle' ? 'Duelo' : 'Ranking'} • {room.options.length} opções</p></button>)}</div></aside></div>}
        ...
        {screen === "room" && currentRoom && <div className="grid gap-6 py-6 lg:grid-cols-[1fr_360px]"><section className="rounded-[2.8rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur-2xl md:p-8"><p className="text-xs font-black uppercase tracking-[0.24em] text-violet-200">Arena • {currentRoom.category} • {currentRoom.mode === 'battle' ? 'Duelo' : 'Ranking'}</p><h1 className="mt-3 text-4xl font-black tracking-[-0.07em] md:text-6xl">{currentRoom.title}</h1>{timer.time === 0 && winner && <div className="mt-6 rounded-[2rem] border border-white/20 bg-white p-5 text-black"><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Campeão da rodada</p><h2 className="mt-2 text-4xl font-black tracking-[-0.07em]">🏆 {winner.text}</h2></div>}<div className={`mt-8 grid gap-4 ${currentRoom.mode === 'battle' ? 'md:grid-cols-2' : ''}`}>{currentRoom.options.map((option) => { const percent = roomVotes ? Math.round((option.votes / roomVotes) * 100) : 0; return <button key={option.id} onClick={()=>vote(option.id)} className="rounded-[2rem] border border-white/10 bg-black/35 p-5 text-left transition hover:-translate-y-1 hover:bg-white/10"><div className="flex items-center justify-between gap-4"><p className="text-2xl font-black tracking-[-0.05em]">{option.text}</p><p className="text-3xl font-black">{option.votes}</p></div><div className="mt-5 h-3 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${percent}%` }} /></div><p className="mt-3 text-xs font-black uppercase tracking-[0.16em] text-zinc-500">{percent}% dos votos</p></button> })}</div></section><aside className="rounded-[2.8rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur-2xl"><p className="text-xs font-black uppercase tracking-[0.24em] text-zinc-500">Timer</p><h2 className="mt-3 text-7xl font-black tracking-[-0.09em]">{timer.time}s</h2><p className="mt-2 text-sm text-zinc-400">{timer.running ? 'Rodada ativa' : timer.time === 0 ? 'Rodada encerrada' : 'Pronto'}</p><div className="mt-6 rounded-3xl bg-black/35 p-5"><p className="text-sm text-zinc-500">Líder</p><p className="mt-2 text-3xl font-black tracking-[-0.06em]">{winner?.text ?? 'Sem líder'}</p><p className="mt-2 text-sm text-zinc-500">{roomVotes} votos totais</p></div><button onClick={timer.start} className="mt-6 w-full rounded-2xl bg-white px-5 py-4 text-sm font-black uppercase tracking-[0.16em] text-black">Iniciar 30s</button><button onClick={saveWinner} className="mt-3 w-full rounded-2xl border border-white/10 bg-black/35 px-5 py-4 text-sm font-black uppercase tracking-[0.16em] text-zinc-300">Salvar campeão</button><button onClick={resetVotes} className="mt-3 w-full rounded-2xl border border-white/10 bg-black/35 px-5 py-4 text-sm font-black uppercase tracking-[0.16em] text-zinc-300">Resetar</button></aside></div>}
