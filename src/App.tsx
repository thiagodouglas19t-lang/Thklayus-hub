...
export default function App() {
  ...
  const canDraw = started && turn === "player" && !gameOver && playableCards.length === 0 && deck.length > 0 && !pendingWild;
  const tip = !started ? "Clique em Jogar para iniciar." : pendingWild ? "Escolha uma cor para o WILD." : turn === "bot" ? "Aguarde o rival." : playableCards.length ? `Você tem ${playableCards.length} carta(s) jogável(eis). Melhor: ${recommended?.value}.` : "Nenhuma carta jogável. Agora pode comprar.";
  ...
  function addLog(text: string) { setLog((current) => [text, ...current].slice(0, 6)); }
  function sound(type: "card" | "win" | "bad") {
    if (muted || Date.now() - lastSoundRef.current < 90) return;
  ...
  function reward(c: number, x: number) {
    const nextCoins = coins + c, nextXp = xp + x;
    setCoins(nextCoins); setXp(nextXp);
    localStorage.setItem("colorclash:coins", String(nextCoins)); localStorage.setItem("colorclash:xp", String(nextXp));
  }
  ...
  function startGame() {
    const fresh = makeDeck(), p = draw(fresh, 7), b = draw(p.rest, 7);
    const first = b.rest.find((c) => c.type === "number") || b.rest[0];
    setDeck(b.rest.filter((c) => c.id !== first.id)); setPlayerHand(p.taken); setBotHand(b.taken); setDiscard([first]);
    setTurn("player"); setStarted(true); setCombo(0); setResult(null); setLastPlayed(null); setPendingWild(null); setLog([]);
    setMessage("Sua vez. Jogue uma carta da mesma cor ou valor."); addLog(`Mesa começou com ${colorName[first.color]} ${first.value}`);
  }
  function drawPlayer() {
    if (!canDraw) { if (playableCards.length) setMessage("Você ainda tem carta jogável. Jogue antes de comprar."); sound("bad"); return; }
    const d = draw(deck, 1); setDeck(d.rest); setPlayerHand((h) => [...h, ...d.taken]); setCombo(0); setTurn("bot");
    setMessage("Você comprou. Vez do rival."); addLog(`${displayName} comprou uma carta`); sound("card"); setTimeout(botMove, 700);
  }
  function win(nextHand: Card[]) {
    const nextWins = wins + 1; setWins(nextWins); localStorage.setItem("colorclash:wins", String(nextWins));
    reward(35 + combo * 3, 60 + combo * 5); setResult("win"); setPlayerHand(nextHand); setMessage("Você venceu!"); addLog(`${displayName} venceu a partida`); sound("win");
  }
  function finishPlayerTurn(card: Card, nextHand: Card[]) {
    if (nextHand.length === 0) return win(nextHand);
    if (nextHand.length === 1) setMessage("COLOR! Você ficou com uma carta.");
    if (card.type === "draw2") { const d = draw(deck, 2); setDeck(d.rest); setBotHand((h) => [...h, ...d.taken]); setMessage("+2! Rival comprou. Jogue de novo."); addLog(`${displayName} jogou +2`); return; }
    if (card.type === "skip") { setMessage("BLOQ! Rival perdeu a vez. Jogue de novo."); addLog(`${displayName} jogou BLOQ`); return; }
    setTurn("bot"); setMessage("Vez do rival."); setTimeout(botMove, 800);
  }
  function playCard(card: Card) {
    if (!started || turn !== "player" || gameOver || !topCard || pendingWild) return;
    if (!canPlay(card, topCard)) { setMessage("Essa carta não combina com a mesa."); sound("bad"); return; }
    animate(card); sound("card");
    const nextCombo = combo + 1, nextHand = playerHand.filter((c) => c.id !== card.id);
    setCombo(nextCombo); setPlayerHand(nextHand); reward(1 + Math.floor(nextCombo / 3), 4 + nextCombo);
    if (card.type === "wild") { setPendingWild(card); setMessage("Escolha uma cor para o WILD."); return; }
    setDiscard((d) => [...d, card]); addLog(`${displayName} jogou ${card.value} ${colorName[card.color]}`); finishPlayerTurn(card, nextHand);
  }
  function chooseWildColor(color: Color) {
    if (!pendingWild) return;
    const chosen = { ...pendingWild, color };
    setDiscard((d) => [...d, chosen]); setPendingWild(null); addLog(`${displayName} escolheu ${colorName[color]}`);
    finishPlayerTurn(chosen, playerHand);
  }
  function botMove() {
    setBotHand((hand) => {
      const top = discard[discard.length - 1]; if (!top || result) return hand;
      const options = hand.filter((c) => canPlay(c, top));
      const card = options.find((c) => c.type === "draw2") || options.find((c) => c.type === "skip") || options.find((c) => c.type === "wild") || options[0];
      if (!card) { setDeck((d) => { const buy = draw(d, 1); setBotHand((h) => [...h, ...buy.taken]); return buy.rest; }); setTurn("player"); setMessage("Rival comprou. Sua vez."); addLog("Rival comprou uma carta"); return hand; }
      animate(card); sound("card"); const next = hand.filter((c) => c.id !== card.id); let played = card;
      if (card.type === "wild") { const chosen = chooseBotColor(next); played = { ...card, color: chosen }; addLog(`Rival jogou WILD e escolheu ${colorName[chosen]}`); } else addLog(`Rival jogou ${card.value} ${colorName[card.color]}`);
      setDiscard((d) => [...d, played]);
  ...
  return <main className="min-h-screen bg-[#130c06] p-3 text-white md:p-4"><section className="mx-auto max-w-6xl">
    ...
    {helpOpen && <div className="mb-4 rounded-[2rem] bg-amber-300 p-4 text-black"><p className="font-black">Como jogar</p><p className="mt-2 text-sm">Jogue carta da mesma cor ou valor. Se tiver carta jogável, não pode comprar. BLOQ bloqueia. +2 faz comprar. WILD deixa escolher a cor.</p></div>}
