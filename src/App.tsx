...
const nextPlayer = (id: PlayerId) => playerOrder[(playerOrder.indexOf(id) + 1) % playerOrder.length];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_e, session) => setProfile(session?.user?.email ? { email: session.user.email, id: session.user.id } : null));
  ...
  function startGame() {
    const fresh = makeDeck(), p = draw(fresh, 7), b1 = draw(p.rest, 7), b2 = draw(b1.rest, 7);
    const first = b2.rest.find((c) => c.type === "number") || b2.rest[0];
    setDeck(b2.rest.filter((c) => c.id !== first.id));
    setPlayers([{ id: "you", name: displayName, human: true, hand: p.taken }, { id: "rival1", name: "Rival 1", human: false, hand: b1.taken }, { id: "rival2", name: "Rival 2", human: false, hand: b2.taken }]);
    setDiscard([first]); setTurn("you"); setStarted(true); setCombo(0); setResult(null); setWinnerName(""); setLastPlayed(null); setPendingWild(null); setLog([]);
  ...
  function drawPlayer() { if (!canDraw) { if (playableCards.length) setMessage("Você ainda tem carta jogável. Jogue antes de comprar."); sound("bad"); return; } drawFor("you", 1); setCombo(0); const next = nextPlayer("you"); setTurn(next); setMessage("Você comprou. Vez do rival."); sound("card"); setTimeout(() => botMove(next), 700); }

  function finishTurn(playerId: PlayerId, card: Card, nextHand: Card[]) {
    if (nextHand.length === 0) return endGame(playerId);
    const target = nextPlayer(playerId);
    if (nextHand.length === 1) setMessage(`${getPlayerName(playerId, displayName)} está com uma carta!`);
    if (card.type === "draw2") { drawFor(target, 2); setCombo(0); setMessage(`${getPlayerName(target, displayName)} comprou +2.`); const next = nextPlayer(target); setTurn(next); if (next !== "you") setTimeout(() => botMove(next), 850); return; }
    if (card.type === "skip") { setCombo(0); setMessage(`${getPlayerName(target, displayName)} foi bloqueado.`); const next = nextPlayer(target); setTurn(next); if (next !== "you") setTimeout(() => botMove(next), 850); return; }
    const next = nextPlayer(playerId); setTurn(next); setMessage(next === "you" ? "Sua vez." : `Vez de ${getPlayerName(next, displayName)}.`); if (next !== "you") setTimeout(() => botMove(next), 850);
  }

  function playCard(card: Card) {
    if (!started || turn !== "you" || gameOver || !topCard || pendingWild || !you) return;
    if (!canPlay(card, topCard)) { setMessage("Essa carta não combina com a mesa."); sound("bad"); return; }
    animate(card); sound("card"); const nextCombo = combo + 1, nextHand = you.hand.filter((c) => c.id !== card.id);
    setCombo(nextCombo); updatePlayerHand("you", nextHand); reward(1 + Math.floor(nextCombo / 3), 4 + nextCombo);
    if (card.type === "wild") { setPendingWild(card); setMessage("Escolha uma cor para o WILD."); return; }
    setDiscard((d) => [...d, card]); addLog(`${displayName} jogou ${card.value} ${colorName[card.color]}`); finishTurn("you", card, nextHand);
  }
  ...
  function botMove(botId: PlayerId) {
    if (botId === "you" || result) return;
    setPlayers((list) => {
      const bot = list.find((p) => p.id === botId), top = discard[discard.length - 1]; if (!bot || !top) return list;
      const options = bot.hand.filter((c) => canPlay(c, top));
      const card = options.find((c) => c.type === "draw2") || options.find((c) => c.type === "skip") || options.find((c) => c.type === "wild") || options[0];
      if (!card) { drawFor(botId, 1); const next = nextPlayer(botId); setTurn(next); setMessage(next === "you" ? "Sua vez." : `Vez de ${getPlayerName(next, displayName)}.`); if (next !== "you") setTimeout(() => botMove(next), 850); return list; }
      animate(card); sound("card"); const nextHand = bot.hand.filter((c) => c.id !== card.id); let played = card;
      if (card.type === "wild") { const chosen = chooseBotColor(nextHand); played = { ...card, color: chosen }; addLog(`${bot.name} jogou WILD e escolheu ${colorName[chosen]}`); } else addLog(`${bot.name} jogou ${card.value} ${colorName[card.color]}`);
      setDiscard((d) => [...d, played]); finishTurn(botId, played, nextHand);
      return list.map((p) => p.id === botId ? { ...p, hand: nextHand } : p);
    });
  }
  ...
  return <main className="min-h-screen bg-[#130c06] p-3 text-white md:p-4"><section className="mx-auto max-w-6xl">
    ...
    {helpOpen && <div className="mb-4 rounded-[2rem] bg-amber-300 p-4 text-black"><p className="font-black">Como jogar</p><p className="mt-2 text-sm">Agora são 3 jogadores. Jogue carta da mesma cor ou valor. Se tiver carta jogável, não pode comprar. BLOQ pula o próximo jogador. +2 faz o próximo comprar. WILD deixa escolher a cor.</p></div>}
