...
function cargaHoraria(course: CourseContent) { const match = course.duration.match(/(\d+)h/); return match ? `${match[1]} horas` : course.duration; }
...
function promessa(course: CourseContent) {
  if (course.free) return "Comece grátis e veja se a plataforma combina com você.";
  if (course.price.includes("4,90")) return "Oferta de entrada para aprender rápido e destravar o primeiro resultado.";
  return "Curso completo com prática, projeto final, material e certificado.";
}
...
export default function Cursos() {
  ...
  const cursoDestaque = professionalCourses.find((course) => course.id === "informatica-profissional") ?? professionalCourses[0];
    ...
    <div className="space-y-7">
      <section className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#050507] p-6 shadow-2xl shadow-black/50 md:p-9">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <span className="rounded-full border border-emerald-300/25 bg-emerald-500/10 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-emerald-100">Curso grátis para começar</span>
            <h2 className="mt-6 max-w-3xl text-4xl font-black tracking-[-0.06em] text-white md:text-6xl">Aprenda algo útil hoje. Depois evolua para cursos pagos.</h2>
            <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-300">Entre pelo curso gratuito de informática, conheça a plataforma e avance para IA, sites, APIs, design e formações profissionalizantes.</p>
            <div className="mt-6 flex flex-wrap gap-3"><button onClick={() => comprarOuAcessar(cursoDestaque)} className="rounded-2xl bg-white px-6 py-4 font-black text-black shadow-lg shadow-emerald-500/20 transition hover:scale-[1.03] active:scale-95">Começar grátis</button><button onClick={() => setCursoDetalhes(cursoDestaque)} className="rounded-2xl border border-white/10 bg-white/[0.05] px-6 py-4 font-black text-white transition hover:border-emerald-300/40">Ver o que aprende</button></div>
          </div>
          <div className="overflow-hidden rounded-[2rem] border border-emerald-300/20 bg-emerald-950/30 shadow-2xl shadow-emerald-500/10">
            <div className="relative p-5"><div className="absolute right-5 top-5 text-6xl">{cursoDestaque.hero}</div><p className="inline-flex rounded-full border border-emerald-200/20 bg-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-emerald-100">Porta de entrada</p><h3 className="mt-5 max-w-[78%] text-3xl font-black leading-tight text-white">{cursoDestaque.title}</h3><p className="mt-4 text-sm leading-6 text-emerald-50/75">{cursoDestaque.subtitle}</p></div>
            <div className="grid grid-cols-[1fr_130px] gap-4 border-t border-white/10 p-5"><div><p className="text-xs font-black uppercase tracking-widest text-emerald-200">Você entrega</p><p className="mt-2 text-sm leading-6 text-white/85">{cursoDestaque.finalProject}</p></div><div className="rounded-[1.4rem] bg-white p-4 text-center text-emerald-950"><p className="text-xs font-black uppercase">Preço</p><p className="mt-2 text-2xl font-black">Grátis</p></div></div>
          </div>
        </div>
      </section>

      <section className="grid gap-4 md:grid-cols-4">{[["Entrada grátis", "Informática atrai e cria confiança."], ["Oferta barata", "IA e prompts destravam a primeira compra."], ["Ticket médio", "Sites e APIs aumentam lucro."], ["Projeto final", "Cada curso gera entrega real."]].map(([title, text]) => <div key={title} className="rounded-3xl border border-white/10 bg-black/35 p-5"><h3 className="font-black text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-zinc-500">{text}</p></div>)}</section>
      ...
      <div className="flex items-end justify-between gap-4"><div><p className="text-sm font-black uppercase tracking-[0.2em] text-zinc-500">Catálogo vendedor</p><h2 className="mt-2 text-3xl font-black text-white">Escolha seu próximo curso</h2></div><p className="text-sm font-bold text-zinc-500">{cursosFiltrados.length} encontrados</p></div>
        ...
        {cursosFiltrados.map((course) => (
          <article key={course.id} className="group overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950 shadow-xl shadow-black/25 transition duration-300 hover:-translate-y-2 hover:border-emerald-300/35 hover:shadow-emerald-500/10">
            ...
            <div className="border-t border-white/10 p-5">
              <div className="flex flex-wrap gap-2"><span className="rounded-full border border-white/10 px-3 py-1 text-xs font-black text-zinc-300">{levelLabel[course.level]}</span><span className="rounded-full border border-white/10 px-3 py-1 text-xs font-black text-zinc-300">{aulasDoCurso(course)} aulas</span><span className="rounded-full border border-white/10 px-3 py-1 text-xs font-black text-zinc-300">Projeto final</span></div>
      ...
      {cursoDetalhes && <div className="fixed inset-0 z-[80] grid place-items-end bg-black/75 p-3 backdrop-blur-sm md:place-items-center"><div className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-white/10 bg-zinc-950 shadow-2xl"><div className="relative overflow-hidden bg-[radial-gradient(circle_at_15%_0%,rgba(34,197,94,0.22),transparent_32%),linear-gradient(135deg,#020617,#061827)] p-6 md:p-8"><button onClick={() => setCursoDetalhes(null)} className="absolute right-4 top-4 rounded-2xl border border-white/15 bg-black/25 px-4 py-2 font-black text-white">X</button><p className="text-xs font-black uppercase tracking-[0.24em] text-emerald-100/80">Detalhes da formação</p><h2 className="mt-4 max-w-3xl text-4xl font-black leading-tight text-white md:text-6xl">{cursoDetalhes.title}</h2><p className="mt-4 max-w-2xl text-base leading-7 text-zinc-300">{cursoDetalhes.subtitle}</p><div className="mt-6 grid gap-3 sm:grid-cols-4"><div className="rounded-2xl border border-white/10 bg-black/30 p-4"><p className="text-xs text-zinc-500">Aulas</p><p className="text-2xl font-black text-white">{aulasDoCurso(cursoDetalhes)}</p></div><div className="rounded-2xl border border-white/10 bg-black/30 p-4"><p className="text-xs text-zinc-500">Nível</p><p className="text-2xl font-black text-white">{levelLabel[cursoDetalhes.level]}</p></div><div className="rounded-2xl border border-white/10 bg-black/30 p-4"><p className="text-xs text-zinc-500">Duração</p><p className="text-2xl font-black text-white">{duracaoPrincipal(cursoDetalhes)}</p></div><div className="rounded-2xl bg-white p-4 text-black"><p className="text-xs font-black">Investimento</p><p className="text-2xl font-black">{cursoDetalhes.price}</p></div></div></div><div className="grid gap-6 p-5 md:grid-cols-[1.1fr_0.9fr] md:p-7"><section><h3 className="text-2xl font-black text-white">O que você desenvolve</h3><p className="mt-3 rounded-2xl border border-white/10 bg-black/35 p-5 text-sm leading-7 text-zinc-300">{cursoDetalhes.outcome}</p><h3 className="mt-6 text-2xl font-black text-white">Grade curricular</h3><div className="mt-3 space-y-3">{cursoDetalhes.modules.map((modulo, index) => <div key={modulo.title} className="rounded-2xl border border-white/10 bg-black/35 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">Módulo {index + 1}</p><h4 className="mt-1 font-black text-white">{modulo.title}</h4><p className="mt-2 text-sm text-zinc-400">{modulo.lessons.length} aulas práticas</p></div>)}</div></section><aside className="space-y-4"><div className="rounded-[2rem] border border-white/10 bg-white p-5 text-black"><p className="text-xs font-black uppercase tracking-widest">Por que vale</p><p className="mt-3 text-sm font-black leading-6">Curso com prática, projeto final e certificado liberado ao concluir 100%.</p></div><div className="rounded-[2rem] border border-amber-400/20 bg-amber-500/10 p-5 text-sm text-amber-100"><p className="font-black">Projeto final</p><p className="mt-2 leading-6">{cursoDetalhes.finalProject}</p></div><div className="rounded-[2rem] border border-white/10 bg-black/35 p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Checklist profissional</p><div className="mt-3 space-y-2">{cursoDetalhes.checklist.map((item) => <p key={item} className="text-sm text-zinc-300">✓ {item}</p>)}</div></div><button onClick={() => { comprarOuAcessar(cursoDetalhes); setCursoDetalhes(null); }} className={`w-full rounded-2xl px-5 py-4 text-lg font-black ${cursoDetalhes.free ? "bg-emerald-400 text-black" : "bg-white text-black"}`}>{cursoDetalhes.free ? "Começar grátis" : "Garantir acesso"}</button><button onClick={() => setCursoDetalhes(null)} className="w-full rounded-2xl border border-white/10 px-5 py-4 font-black text-zinc-300">Voltar ao catálogo</button></aside></div></div></div>}
