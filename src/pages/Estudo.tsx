...
export default function Estudo() {
  ...
  const totalLiberados = cursosOrdenados.filter((course) => temAcesso(course.id, course.title, course.free)).length;
  const totalConcluidos = cursosOrdenados.filter((course) => getCourseProgress(course).percent === 100).length;
  ...
  function motivoAcesso(course: Course) {
    ...
    if (manualKeys.has(normalize(course.id)) || manualKeys.has(normalize(course.title))) return "Liberado";
    ...
    return compraAprovada ? "Liberado" : "Bloqueado";
  }
  ...
  function abrirCurso(course: Course) {
    if (!temAcesso(course.id, course.title, course.free)) {
      alert("Item aguardando liberação. Se você enviou comprovante, espere o ADM aprovar o acesso.");
      return;
    }
    ...
    <div className="space-y-3">
      {selecionado.modules.map((modulo, moduloIndex) => (
        <div key={modulo.title} className="rounded-2xl border border-white/10 bg-black/35 p-3">
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500">Parte {moduloIndex + 1}</p>
                ...
                >
                  <span
                    className={`grid h-6 w-6 shrink-0 place-items-center rounded-lg text-[10px] font-black ${
                      done ? "bg-violet-300 text-black" : active ? "bg-black text-white" : "bg-white/10 text-zinc-500"
                    }`}
                  >
                    {done ? "✓" : aulaIndex + 1}
                  </span>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-violet-300 transition-all" style={{ width: `${porcentagemSelecionado}%` }} />
              </div>
            </div>
            <button onClick={() => setShowLessons(true)} className="rounded-2xl border border-white/10 px-3 py-3 text-xs font-black text-zinc-300 lg:hidden">Partes</button>
          ...
          <div className="fixed inset-0 z-[90] bg-black/80 p-3 backdrop-blur-xl lg:hidden">
            <div className="mx-auto max-h-[92vh] max-w-lg overflow-y-auto rounded-[2rem] border border-white/10 bg-zinc-950 p-4">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-xl font-black">Partes do item</h3>
        ...
        <main className="grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
          <aside className="hidden rounded-[2rem] border border-white/10 bg-zinc-950 p-4 lg:sticky lg:top-40 lg:block lg:max-h-[72vh] lg:overflow-y-auto">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              <div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Tempo</p><p className="text-sm font-black">{selecionado.duration}</p></div>
              <div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Partes</p><p className="text-sm font-black">{concluidasSelecionado}/{totalAulas}</p></div>
            ...
            <h3 className="mt-6 text-lg font-black text-white">Estrutura</h3>
          ...
          <section className="space-y-4">
            <div className="rounded-[2rem] border border-white/10 bg-zinc-950 p-4 lg:hidden">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-zinc-500">{selecionado.duration} • {levelLabel[selecionado.level]} • {concluidasSelecionado}/{totalAulas} partes</p>
                </div>
              </div>
            </div>
            ...
            {porcentagemSelecionado === 100 && (
              <div className="rounded-[2rem] border border-violet-400/20 bg-violet-500/10 p-5">
                <p className="text-4xl">🏆</p>
                <h2 className="mt-3 text-2xl font-black text-violet-100">Item concluído!</h2>
                <p className="mt-2 text-sm text-violet-100/70">Seu certificado/registro de conclusão está liberado acima.</p>
              </div>
            )}
            ...
            {currentLesson && (
              <article className="overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950 shadow-2xl shadow-black/40">
                <div className="border-b border-white/10 bg-white p-5 text-black md:p-7">
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Parte {currentLesson.moduleIndex + 1} • Bloco {currentLesson.lessonIndex + 1} de {lessonList.length}</p>
                ...
                <div className="grid gap-3 p-4 md:p-6">
                  <section className="rounded-[1.5rem] border border-white/10 bg-black p-5">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-300">📖 Explicação</p>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <section className="rounded-[1.5rem] border border-white/10 bg-black p-5">
                      <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-300">🧪 Exemplo prático</p>
                      <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-zinc-300">{currentLesson.lesson.example ?? "Exemplo prático disponível na explicação."}</p>
                    </section>
                    <section className="rounded-[1.5rem] border border-white/10 bg-black p-5">
                      <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-300">🎯 Resultado esperado</p>
                      <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-zinc-300">{currentLesson.lesson.expectedResult ?? "Ao final, você deve conseguir aplicar o conteúdo em uma entrega simples."}</p>
                    </section>
                  </div>

                  <section className="rounded-[1.5rem] border border-white/10 bg-black p-5">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-300">🧭 Passo a passo</p>
                  ...
                  <section className="rounded-[1.5rem] border border-red-400/20 bg-red-500/5 p-5">
                    ...
                    <div className="mt-4 space-y-3">
                      {(currentLesson.lesson.commonMistakes ?? ["Pular a prática e apenas ler. Evite isso fazendo a atividade proposta.", "Copiar sem entender. Evite isso explicando com suas palavras o que você fez."]).map((mistake) => (
                        <p key={mistake} className="text-sm leading-7 text-red-50/80">• {mistake}</p>
                      ))}
                    </div>
                  </section>

                  <section className="rounded-[1.5rem] border border-violet-400/20 bg-violet-500/5 p-5">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-200">✅ Exercício prático</p>
                    <p className="mt-4 whitespace-pre-wrap text-base leading-8 text-violet-50/85">{currentLesson.lesson.exercise ?? currentLesson.lesson.practice}</p>
                  </section>

                  <div className="grid gap-3 md:grid-cols-2">
                    <button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className={`rounded-2xl px-5 py-4 font-black transition active:scale-95 ${currentDone ? "border border-violet-400/30 bg-violet-500/10 text-violet-100" : "bg-white text-black"}`}>{currentDone ? "Concluído ✓" : "Marcar como concluído"}</button>
                    <button onClick={concluirEProxima} className="rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4 font-black text-white transition hover:border-violet-300/40 active:scale-95">{nextLesson ? "Concluir e ir para próxima" : "Concluir item"}</button>
                  </div>

                  {nextLesson && <button onClick={() => setLesson(nextLesson.moduleIndex, nextLesson.lessonIndex)} className="rounded-2xl border border-white/10 bg-black px-5 py-4 text-left font-black text-zinc-300 transition hover:border-white/30">Próxima parte: {nextLesson.lesson.title}</button>}
                </div>
              </article>
            )}
          </section>
  ...
  if (loading) {
    return (
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-8 text-center">
        <p className="text-2xl font-black text-white">Carregando seus itens...</p>
        <p className="mt-2 text-sm text-zinc-500">Verificando acessos e materiais liberados.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-28">
      <section className="relative overflow-hidden rounded-[3rem] border border-violet-300/15 bg-[#030006] px-6 py-10 shadow-2xl shadow-violet-950/25 md:px-10 md:py-14">
        ...
        <div className="relative max-w-4xl">
          <span className="rounded-full border border-violet-300/25 bg-violet-500/10 px-4 py-2 text-[11px] font-black uppercase tracking-[0.22em] text-violet-100">Meus itens</span>
          <h1 className="mt-7 text-5xl font-black leading-[0.95] tracking-[-0.08em] text-white md:text-7xl">Tudo que você salvou ou liberou.</h1>
          <p className="mt-6 max-w-2xl text-base leading-8 text-zinc-300">Acesse modelos, materiais e extras disponíveis na sua conta. O foco é usar rápido, continuar depois e acompanhar seu progresso.</p>
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"><p className="text-sm text-zinc-500">Liberados</p><p className="mt-1 text-3xl font-black text-white">{totalLiberados}</p></div>
            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"><p className="text-sm text-zinc-500">Concluídos</p><p className="mt-1 text-3xl font-black text-white">{totalConcluidos}</p></div>
            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"><p className="text-sm text-zinc-500">Disponíveis</p><p className="mt-1 text-3xl font-black text-white">{professionalCourses.length}</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[2.5rem] border border-white/10 bg-white/[0.03] p-4 backdrop-blur-xl">
        <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar item, modelo ou categoria..." className="min-h-12 rounded-2xl border border-white/10 bg-black/45 px-4 text-sm font-bold text-white outline-none placeholder:text-zinc-600 focus:border-violet-300/40" />
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {cursosFiltrados.map((course) => {
          ...
          return (
            <article key={course.id} className="group overflow-hidden rounded-[2.3rem] border border-white/10 bg-[#07070a] shadow-xl shadow-black/25 transition duration-300 hover:-translate-y-1 hover:border-violet-300/35">
              <div className="relative bg-[radial-gradient(circle_at_20%_0%,rgba(168,85,247,0.18),transparent_35%),linear-gradient(135deg,#06020f,#0b1220)] p-5">
                <div className="absolute right-4 top-4 text-5xl opacity-90">{course.hero}</div>
                <span className={`rounded-full px-3 py-1 text-xs font-black ${acesso ? "bg-violet-300 text-black" : "bg-white/10 text-zinc-400"}`}>{motivoAcesso(course)}</span>
                <h2 className="mt-5 max-w-[82%] text-3xl font-black leading-tight text-white">{course.title}</h2>
                <p className="mt-3 max-w-[90%] text-sm leading-6 text-zinc-400">{course.subtitle}</p>
              </div>
              <div className="p-5">
                <div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-violet-300 transition-all" style={{ width: `${courseProgress.percent}%` }} /></div>
                <p className="mt-2 text-xs font-bold text-zinc-500">{courseProgress.done}/{courseProgress.total} partes • {courseProgress.percent}%</p>
                <button onClick={() => abrirCurso(course)} className={`mt-5 w-full rounded-2xl px-5 py-4 font-black transition active:scale-95 ${acesso ? "bg-white text-black" : "border border-white/10 text-zinc-500"}`}>{acesso ? "Abrir item" : "Aguardando liberação"}</button>
              </div>
            </article>
          );
        })}
      ...
      {cursosFiltrados.length === 0 && <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-8 text-center text-zinc-500">Nenhum item encontrado.</div>}
    </div>
  );
}
