  ...
  if (view === "course") {
    return (
      <div className="space-y-5 pb-28">
        <section className="sticky top-[76px] z-30 rounded-[1.5rem] border border-white/10 bg-black/85 p-3 shadow-2xl shadow-black/60 backdrop-blur-2xl"><div className="flex items-center justify-between gap-3"><button onClick={() => setView("catalog")} className="rounded-2xl border border-white/10 px-4 py-3 text-xs font-black text-zinc-300">← Meus</button><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{selecionado.title}</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${porcentagemSelecionado}%` }} /></div></div><span className="rounded-2xl bg-white px-3 py-2 text-xs font-black text-black">{porcentagemSelecionado}%</span></div></section>
        <main className="grid gap-5 lg:grid-cols-[0.78fr_1.22fr]"><aside className="rounded-[2rem] border border-white/10 bg-zinc-950 p-4 lg:sticky lg:top-40 lg:max-h-[72vh] lg:overflow-y-auto"><div className="rounded-[1.5rem] bg-white p-5 text-black"><p className="text-5xl">{selecionado.hero}</p><p className="mt-4 break-all text-xs font-black uppercase tracking-[0.18em] text-zinc-500">ID: {selecionado.id}</p><h1 className="mt-2 text-2xl font-black leading-tight">{selecionado.title}</h1><p className="mt-2 text-sm font-semibold leading-6 text-zinc-700">{selecionado.subtitle}</p></div><div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Duração</p><p className="text-sm font-black">{selecionado.duration}</p></div><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Aulas</p><p className="text-sm font-black">{concluidasSelecionado}/{totalAulas}</p></div><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Nível</p><p className="text-sm font-black">{levelLabel[selecionado.level]}</p></div></div><div className="mt-4 grid gap-2"><button onClick={baixarMaterial} className="w-full rounded-2xl bg-white px-5 py-3 font-black text-black">Baixar material</button><button onClick={baixarCertificado} className={`w-full rounded-2xl px-5 py-3 font-black ${porcentagemSelecionado === 100 ? "bg-emerald-400 text-black" : "border border-white/10 text-zinc-500"}`}>Certificado</button></div><h3 className="mt-6 text-lg font-black text-white">Aulas</h3><div className="mt-3 space-y-3">{selecionado.modules.map((modulo, moduloIndex) => <div key={modulo.title} className="rounded-2xl border border-white/10 bg-black/35 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500">Módulo {moduloIndex + 1}</p><p className="mt-1 text-sm font-black text-zinc-200">{modulo.title}</p><div className="mt-3 space-y-2">{modulo.lessons.map((aula, aulaIndex) => { const key = lessonKey(selecionado.id, moduloIndex, aulaIndex); const done = progress[selecionado.id]?.includes(key) ?? false; const active = activeLesson.moduleIndex === moduloIndex && activeLesson.lessonIndex === aulaIndex; return <button key={aula.title} onClick={() => setLesson(moduloIndex, aulaIndex)} className={`flex w-full items-center gap-3 rounded-xl border px-3 py-2 text-left text-xs font-bold transition ${active ? "border-white bg-white text-black" : "border-white/10 bg-zinc-950 text-zinc-400"}`}><span className={`grid h-6 w-6 shrink-0 place-items-center rounded-lg text-[10px] font-black ${done ? "bg-emerald-400 text-black" : active ? "bg-black text-white" : "bg-white/10 text-zinc-500"}`}>{done ? "✓" : aulaIndex + 1}</span><span className="line-clamp-1">{aula.title}</span></button>; })}</div></div>)}</div></aside>
          <section className="space-y-5">{porcentagemSelecionado === 100 && <div className="rounded-[2rem] border border-emerald-400/20 bg-emerald-500/10 p-5"><p className="text-4xl">🏆</p><h2 className="mt-3 text-2xl font-black text-emerald-100">Curso concluído!</h2><p className="mt-2 text-sm text-emerald-100/70">Certificado liberado. Baixe seu certificado no botão ao lado.</p><button onClick={baixarCertificado} className="mt-4 rounded-2xl bg-emerald-400 px-5 py-3 font-black text-black">Baixar certificado</button></div>}{currentLesson && <article className="overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950 shadow-2xl shadow-black/40"><div className="border-b border-white/10 bg-white p-5 text-black md:p-7"><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Módulo {currentLesson.moduleIndex + 1} • Aula {currentLesson.lessonIndex + 1} de {lessonList.length}</p><h2 className="mt-3 text-4xl font-black leading-tight md:text-6xl">{currentLesson.lesson.title}</h2><p className="mt-3 text-sm font-semibold text-zinc-600">{currentLesson.modulo.title}</p></div><div className="grid gap-4 p-4 md:p-6"><section className="rounded-[1.5rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-300">📖 Explicação</p><p className="mt-4 whitespace-pre-wrap text-base leading-8 text-zinc-200">{currentLesson.lesson.summary}</p></section><section className="rounded-[1.5rem] border border-emerald-400/20 bg-emerald-500/10 p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-200">🧪 Prática guiada</p><p className="mt-4 text-base leading-8 text-emerald-50/90">{currentLesson.lesson.practice}</p></section><section className="rounded-[1.5rem] border border-amber-400/20 bg-amber-500/10 p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-200">🎯 Resultado esperado</p><p className="mt-4 text-base leading-8 text-amber-50/90">{selecionado.outcome}</p></section><div className="grid gap-3 sm:grid-cols-3"><button onClick={() => currentIndex > 0 && setLesson(lessonList[currentIndex - 1].moduleIndex, lessonList[currentIndex - 1].lessonIndex)} disabled={currentIndex <= 0} className="rounded-2xl border border-white/10 px-5 py-4 font-black text-zinc-300 disabled:opacity-40">Aula anterior</button><button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className={`rounded-2xl px-5 py-4 font-black ${currentDone ? "bg-emerald-400 text-black" : "bg-white text-black"}`}>{currentDone ? "Concluída ✓" : "Marcar concluída"}</button><button onClick={concluirEProxima} className="rounded-2xl border border-blue-400/30 bg-blue-500/10 px-5 py-4 font-black text-blue-100">{currentIndex >= lessonList.length - 1 ? "Finalizar" : "Próxima aula"}</button></div></div></article>}<section className="grid gap-4 md:grid-cols-2"><div className="rounded-[2rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Projeto final</p><p className="mt-3 text-sm leading-7 text-zinc-300">{selecionado.finalProject}</p></div><div className="rounded-[2rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Checklist</p><div className="mt-3 space-y-2">{selecionado.checklist.map((item) => <p key={item} className="text-sm text-zinc-300">✓ {item}</p>)}</div></div></section></section></main>{currentLesson && <div className="fixed bottom-[92px] left-3 right-3 z-40 rounded-[1.5rem] border border-white/10 bg-black/90 p-3 shadow-2xl shadow-black/80 backdrop-blur-2xl lg:hidden"><div className="flex items-center gap-2"><button onClick={() => setView("catalog")} className="rounded-xl border border-white/10 px-3 py-3 text-xs font-black text-zinc-300">Meus</button><button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className="flex-1 rounded-xl bg-white px-3 py-3 text-xs font-black text-black">{currentDone ? "Concluída ✓" : "Concluir"}</button><button onClick={concluirEProxima} className="flex-1 rounded-xl border border-blue-400/30 bg-blue-500/10 px-3 py-3 text-xs font-black text-blue-100">Próxima</button></div></div>}
      </div>
    );
  }
