...
function normalize(text?: string | null) { return (text ?? "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim(); }
...
function clampLesson(course: Course, saved?: ActiveLesson): ActiveLesson { const list = flatLessons(course); if (!saved) return { moduleIndex: 0, lessonIndex: 0 }; return list.some((item) => item.moduleIndex === saved.moduleIndex && item.lessonIndex === saved.lessonIndex) ? saved : { moduleIndex: 0, lessonIndex: 0 }; }
...
export default function Estudo() {
  ...
  const manualKeys = useMemo(() => new Set(acessosManuais.flatMap((item) => [normalize(item.produto_id), normalize(item.course_title)]).filter(Boolean)), [acessosManuais]);
  ...
  const cursosOrdenados = useMemo(() => [...professionalCourses].sort((a, b) => Number(temAcesso(b.id, b.title, b.free)) - Number(temAcesso(a.id, a.title, a.free))), [liberadas, manualKeys]);
  ...
  function motivoAcesso(course: Course) {
    ...
    if (liberadas.some((compra) => normalize(compra.course_id) === normalize(course.id) || normalize(compra.course_title) === normalize(course.title))) return "Compra aprovada";
    return "Bloqueado";
  }
  ...
  function abrirCurso(course: Course) {
    if (!temAcesso(course.id, course.title, course.free)) { alert("Curso aguardando liberação. Envie o comprovante e espere o ADM aprovar o acesso."); return; }
  ...
  if (view === "course") {
    ...
    return (
      <div className="space-y-4 pb-28">
        <section className="sticky top-[76px] z-30 rounded-[1.5rem] border border-white/10 bg-black/90 p-3 shadow-2xl shadow-black/60 backdrop-blur-2xl"><div className="flex items-center justify-between gap-2"><button onClick={() => setView("catalog")} className="rounded-2xl border border-white/10 px-3 py-3 text-xs font-black text-zinc-300">←</button><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{selecionado.title}</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${porcentagemSelecionado}%` }} /></div></div><button onClick={() => setShowLessons(true)} className="rounded-2xl border border-white/10 px-3 py-3 text-xs font-black text-zinc-300 lg:hidden">Aulas</button><span className="rounded-2xl bg-white px-3 py-3 text-xs font-black text-black">{porcentagemSelecionado}%</span></div></section>
        ...
        <main className="grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
          ...
          <section className="space-y-4"><div className="rounded-[2rem] border border-white/10 bg-zinc-950 p-4 lg:hidden"><div className="flex items-start gap-3"><div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-white text-3xl">{selecionado.hero}</div><div className="min-w-0"><p className="break-all text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500">ID: {selecionado.id}</p><h1 className="mt-1 text-xl font-black leading-tight">{selecionado.title}</h1><p className="mt-1 text-xs leading-5 text-zinc-500">{selecionado.duration} • {levelLabel[selecionado.level]} • {concluidasSelecionado}/{totalAulas} aulas</p></div></div></div>{porcentagemSelecionado === 100 && <div className="rounded-[2rem] border border-emerald-400/20 bg-emerald-500/10 p-5"><p className="text-4xl">🏆</p><h2 className="mt-3 text-2xl font-black text-emerald-100">Curso concluído!</h2><p className="mt-2 text-sm text-emerald-100/70">Certificado liberado.</p><button onClick={baixarCertificado} className="mt-4 rounded-2xl bg-emerald-400 px-5 py-3 font-black text-black">Baixar certificado</button></div>}{currentLesson && <article className="overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950 shadow-2xl shadow-black/40"><div className="border-b border-white/10 bg-white p-5 text-black md:p-7"><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Módulo {currentLesson.moduleIndex + 1} • Aula {currentLesson.lessonIndex + 1} de {lessonList.length}</p><h2 className="mt-3 text-3xl font-black leading-tight md:text-6xl">{currentLesson.lesson.title}</h2><p className="mt-3 text-sm font-semibold text-zinc-600">{currentLesson.modulo.title}</p></div><div className="grid gap-3 p-4 md:p-6"><section className="rounded-[1.5rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-300">📖 Explicação</p><p className="mt-4 whitespace-pre-wrap text-base leading-8 text-zinc-200">{currentLesson.lesson.summary}</p></section><div className="grid gap-3 md:grid-cols-2"><section className="rounded-[1.5rem] border border-emerald-400/20 bg-emerald-500/10 p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-200">🧪 Prática guiada</p><p className="mt-4 text-sm leading-7 text-emerald-50/90">{currentLesson.lesson.practice}</p></section><section className="rounded-[1.5rem] border border-blue-400/20 bg-blue-500/10 p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-200">🚀 Próximo passo</p><p className="mt-4 text-sm leading-7 text-blue-50/90">{nextLesson ? `Depois desta aula, siga para: ${nextLesson.lesson.title}.` : "Finalize o projeto final e baixe seu certificado."}</p></section></div><section className="rounded-[1.5rem] border border-amber-400/20 bg-amber-500/10 p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-200">🎯 Resultado esperado</p><p className="mt-4 text-sm leading-7 text-amber-50/90">{selecionado.outcome}</p></section><div className="hidden gap-3 sm:grid sm:grid-cols-3"><button onClick={() => currentIndex > 0 && setLesson(lessonList[currentIndex - 1].moduleIndex, lessonList[currentIndex - 1].lessonIndex)} disabled={currentIndex <= 0} className="rounded-2xl border border-white/10 px-5 py-4 font-black text-zinc-300 disabled:opacity-40">Anterior</button><button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className={`rounded-2xl px-5 py-4 font-black ${currentDone ? "bg-emerald-400 text-black" : "bg-white text-black"}`}>{currentDone ? "Concluída ✓" : "Marcar concluída"}</button><button onClick={concluirEProxima} className="rounded-2xl border border-blue-400/30 bg-blue-500/10 px-5 py-4 font-black text-blue-100">{currentIndex >= lessonList.length - 1 ? "Finalizar" : "Próxima aula"}</button></div></div></article>}<section className="grid gap-3 md:grid-cols-2"><div className="rounded-[1.6rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Projeto final</p><p className="mt-3 text-sm leading-7 text-zinc-300">{selecionado.finalProject}</p></div><div className="rounded-[1.6rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Checklist</p><div className="mt-3 space-y-2">{selecionado.checklist.map((item) => <p key={item} className="text-sm text-zinc-300">✓ {item}</p>)}</div></div></section></section>
        </main>{currentLesson && <div className="fixed bottom-[92px] left-3 right-3 z-40 rounded-[1.5rem] border border-white/10 bg-black/90 p-2 shadow-2xl shadow-black/80 backdrop-blur-2xl lg:hidden"><div className="grid grid-cols-[0.8fr_1.25fr_1.25fr] gap-2"><button onClick={() => setShowLessons(true)} className="rounded-xl border border-white/10 px-3 py-3 text-xs font-black text-zinc-300">Aulas</button><button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className="rounded-xl bg-white px-3 py-3 text-xs font-black text-black">{currentDone ? "Feita ✓" : "Concluir"}</button><button onClick={concluirEProxima} className="rounded-xl border border-blue-400/30 bg-blue-500/10 px-3 py-3 text-xs font-black text-blue-100">Próxima</button></div></div>}
      </div>
    );
  }

  const liberadosCount = professionalCourses.filter((course) => temAcesso(course.id, course.title, course.free)).length;
  const manuaisCount = professionalCourses.filter((course) => manualKeys.has(normalize(course.id)) || manualKeys.has(normalize(course.title))).length;
  const cursoAtivo = cursosOrdenados.find((course) => temAcesso(course.id, course.title, course.free) && getCourseProgress(course).percent > 0) ?? cursosOrdenados.find((course) => temAcesso(course.id, course.title, course.free));

  return <div className="space-y-6"><section className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#050507] p-6 shadow-2xl shadow-black/50 md:p-8"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(124,58,237,0.26),transparent_34%),radial-gradient(circle_at_95%_10%,rgba(56,189,248,0.16),transparent_30%),radial-gradient(circle_at_70%_100%,rgba(16,185,129,0.12),transparent_36%)]" /><div className="relative grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><span className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-zinc-300">Área do aluno</span><h2 className="mt-5 text-4xl font-black tracking-[-0.04em] md:text-6xl">Meus cursos</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-zinc-400">Cursos liberados ficam primeiro. Bloqueados aparecem depois para compra ou liberação pelo ADM.</p><div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4"><div className="rounded-2xl border border-white/10 bg-black/40 p-4"><p className="text-xs text-zinc-500">Liberados</p><p className="mt-1 text-3xl font-black">{liberadosCount}</p></div><div className="rounded-2xl border border-white/10 bg-black/40 p-4"><p className="text-xs text-zinc-500">No app</p><p className="mt-1 text-3xl font-black">{professionalCourses.length}</p></div><div className="rounded-2xl border border-white/10 bg-black/40 p-4"><p className="text-xs text-zinc-500">ADM</p><p className="mt-1 text-3xl font-black">{manuaisCount}</p></div><div className="rounded-2xl border border-white/10 bg-black/40 p-4"><p className="text-xs text-zinc-500">Status</p><p className="mt-1 text-2xl font-black">{loading ? "Verificando" : "Pronto"}</p></div></div></div>{cursoAtivo && <button onClick={() => abrirCurso(cursoAtivo)} className="rounded-[2rem] border border-white/10 bg-white p-5 text-left text-black shadow-2xl shadow-blue-500/10 transition hover:-translate-y-1"><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Continuar aprendizado</p><div className="mt-4 flex items-start gap-4"><div className="text-5xl">{cursoAtivo.hero}</div><div><h3 className="text-2xl font-black leading-tight">{cursoAtivo.title}</h3><p className="mt-2 text-sm font-semibold text-zinc-600">{getCourseProgress(cursoAtivo).done}/{getCourseProgress(cursoAtivo).total} aulas • {getCourseProgress(cursoAtivo).percent}%</p></div></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-zinc-200"><div className="h-full bg-black" style={{ width: `${getCourseProgress(cursoAtivo).percent}%` }} /></div><p className="mt-4 inline-flex rounded-2xl bg-black px-5 py-3 font-black text-white">Continuar agora</p></button>}</div></section><section className="rounded-[2rem] border border-white/10 bg-zinc-950 p-4 md:p-5"><div className="grid gap-3 md:grid-cols-[1fr_auto]"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar por nome ou ID do curso..." className="w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm font-bold text-white outline-none placeholder:text-zinc-600 focus:border-white/30" /><button onClick={() => { setQuery(""); setCategoria("Todos"); }} className="rounded-2xl border border-white/10 px-5 py-3 text-sm font-black text-zinc-300 transition hover:bg-white/10">Limpar filtros</button></div><div className="mt-4 flex gap-2 overflow-x-auto pb-1">{categorias.map((item) => <button key={item} onClick={() => setCategoria(item)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-black transition ${categoria === item ? "bg-white text-black" : "border border-zinc-800 bg-black text-zinc-400 hover:border-white/20"}`}>{item}</button>)}</div></section>{cursosFiltrados.length === 0 ? <section className="rounded-[2rem] border border-white/10 bg-zinc-950 p-8 text-center"><p className="text-5xl">🔎</p><h3 className="mt-4 text-2xl font-black">Nenhum curso encontrado</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-zinc-500">Tente pesquisar outro nome ou limpar os filtros.</p></section> : <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{cursosFiltrados.map((course) => { const acesso = temAcesso(course.id, course.title, course.free); const courseProgress = getCourseProgress(course); return <button key={course.id} onClick={() => abrirCurso(course)} className={`group overflow-hidden rounded-[2rem] border bg-zinc-950 text-left transition hover:-translate-y-1 ${acesso ? "border-white/10 hover:border-emerald-300/35" : "border-white/5 opacity-50 hover:opacity-80"}`}><div className="border-b border-white/10 bg-white p-5 text-black"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">{course.category}</p><h3 className="mt-3 text-2xl font-black leading-tight">{course.title}</h3></div><p className="text-5xl">{course.hero}</p></div></div><div className="space-y-4 p-5"><div className="flex flex-wrap gap-2"><span className="rounded-full border border-white/10 px-3 py-1 text-xs font-black text-zinc-300">{levelLabel[course.level]}</span><span className="rounded-full border border-white/10 px-3 py-1 text-xs font-black text-zinc-300">{courseProgress.total} aulas</span><span className={`rounded-full px-3 py-1 text-xs font-black ${acesso ? "bg-white text-black" : "border border-amber-800 text-amber-200"}`}>{motivoAcesso(course)}</span></div><p className="text-sm leading-6 text-zinc-400">{course.subtitle}</p><div><div className="mb-2 flex items-center justify-between text-xs font-black text-zinc-500"><span>Progresso</span><span>{courseProgress.done}/{courseProgress.total} • {courseProgress.percent}%</span></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${courseProgress.percent}%` }} /></div></div><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-xs text-zinc-500">Duração</p><p className="font-black">{course.duration}</p></div><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-xs text-zinc-500">Preço</p><p className="font-black">{course.price}</p></div></div><div className="flex items-center justify-between gap-3 border-t border-white/10 pt-4"><span className={`rounded-full px-4 py-2 text-xs font-black ${acesso ? "bg-white text-black" : "border border-amber-800 text-amber-200"}`}>{acesso ? (courseProgress.percent > 0 ? "Continuar" : "Começar") : "Bloqueado"}</span><span className="text-xs font-bold text-zinc-500">{acesso ? (courseProgress.percent === 100 ? "Concluído" : "Em andamento") : "Peça liberação"}</span></div></div></button>; })}</section>}</div>;
}
