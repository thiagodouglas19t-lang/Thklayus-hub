  ...
  if (view === "course") {
    return (
      <div className="space-y-5 pb-28">
        <section className="sticky top-[76px] z-30 rounded-[1.5rem] border border-white/10 bg-black/85 p-3 shadow-2xl shadow-black/60 backdrop-blur-2xl">
          <div className="flex items-center justify-between gap-3">
            <button onClick={() => setView("catalog")} className="rounded-2xl border border-white/10 px-4 py-3 text-xs font-black text-zinc-300">← Meus</button>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-black text-white">{selecionado.title}</p>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${porcentagemSelecionado}%` }} /></div>
            </div>
            <span className="rounded-2xl bg-white px-3 py-2 text-xs font-black text-black">{porcentagemSelecionado}%</span>
          </div>
        </section>

        <main className="grid gap-5 lg:grid-cols-[0.78fr_1.22fr]">
          <aside className="rounded-[2rem] border border-white/10 bg-zinc-950 p-4 lg:sticky lg:top-40 lg:max-h-[72vh] lg:overflow-y-auto">
            ...
            <button onClick={baixarMaterial} className="mt-4 w-full rounded-2xl bg-white px-5 py-3 font-black text-black">Baixar material</button>
          ...
          <section className="space-y-5">
            {porcentagemSelecionado === 100 && <div className="rounded-[2rem] border border-emerald-400/20 bg-emerald-500/10 p-5"><p className="text-4xl">🏆</p><h2 className="mt-3 text-2xl font-black text-emerald-100">Curso concluído!</h2><p className="mt-2 text-sm text-emerald-100/70">Todas as aulas foram marcadas como concluídas.</p></div>}
            {currentLesson && (
              <article className="overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950 shadow-2xl shadow-black/40">
                <div className="border-b border-white/10 bg-white p-5 text-black md:p-7">
                  ...
                  <h2 className="mt-3 text-4xl font-black leading-tight md:text-6xl">{currentLesson.lesson.title}</h2>
                ...
                <div className="grid gap-4 p-4 md:p-6">
                  ...
                  <section className="rounded-[1.5rem] border border-emerald-400/20 bg-emerald-500/10 p-5">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-200">🧪 Prática guiada</p>
                    <p className="mt-4 text-base leading-8 text-emerald-50/90">{currentLesson.lesson.practice}</p>
                  </section>
                  <section className="rounded-[1.5rem] border border-amber-400/20 bg-amber-500/10 p-5">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-200">🎯 Resultado esperado</p>
                    <p className="mt-4 text-base leading-8 text-amber-50/90">{selecionado.outcome}</p>
                  </section>
                  <div className="grid gap-3 sm:grid-cols-3">
                    <button onClick={() => currentIndex > 0 && setLesson(lessonList[currentIndex - 1].moduleIndex, lessonList[currentIndex - 1].lessonIndex)} disabled={currentIndex <= 0} className="rounded-2xl border border-white/10 px-5 py-4 font-black text-zinc-300 disabled:opacity-40">Aula anterior</button>
                    <button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className={`rounded-2xl px-5 py-4 font-black ${currentDone ? "bg-emerald-400 text-black" : "bg-white text-black"}`}>{currentDone ? "Concluída ✓" : "Marcar concluída"}</button>
                    <button onClick={concluirEProxima} className="rounded-2xl border border-blue-400/30 bg-blue-500/10 px-5 py-4 font-black text-blue-100">{currentIndex >= lessonList.length - 1 ? "Finalizar" : "Próxima aula"}</button>
                  </div>
                </div>
              </article>
            )}
            <section className="grid gap-4 md:grid-cols-2"><div className="rounded-[2rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Projeto final</p><p className="mt-3 text-sm leading-7 text-zinc-300">{selecionado.finalProject}</p></div><div className="rounded-[2rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Checklist</p><div className="mt-3 space-y-2">{selecionado.checklist.map((item) => <p key={item} className="text-sm text-zinc-300">✓ {item}</p>)}</div></div></section>
          </section>
        </main>

        {currentLesson && <div className="fixed bottom-[92px] left-3 right-3 z-40 rounded-[1.5rem] border border-white/10 bg-black/90 p-3 shadow-2xl shadow-black/80 backdrop-blur-2xl lg:hidden"><div className="flex items-center gap-2"><button onClick={() => setView("catalog")} className="rounded-xl border border-white/10 px-3 py-3 text-xs font-black text-zinc-300">Meus</button><button onClick={() => toggleLesson(selecionado.id, currentLesson.key)} className="flex-1 rounded-xl bg-white px-3 py-3 text-xs font-black text-black">{currentDone ? "Concluída ✓" : "Concluir aula"}</button><button onClick={concluirEProxima} className="flex-1 rounded-xl border border-blue-400/30 bg-blue-500/10 px-3 py-3 text-xs font-black text-blue-100">Próxima</button></div></div>}
      </div>
    );
  }
