      ...
      <div className="space-y-4 pb-28">
        ...
        <main className="grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
          <aside className="hidden rounded-[2rem] border border-white/10 bg-zinc-950 p-4 lg:sticky lg:top-40 lg:block lg:max-h-[72vh] lg:overflow-y-auto">
            <div className="rounded-[1.5rem] bg-white p-5 text-black"><p className="text-5xl">{selecionado.hero}</p><p className="mt-4 break-all text-xs font-black uppercase tracking-[0.18em] text-zinc-500">ID: {selecionado.id}</p><h1 className="mt-2 text-2xl font-black leading-tight">{selecionado.title}</h1><p className="mt-2 text-sm font-semibold leading-6 text-zinc-700">{selecionado.subtitle}</p></div>
            <div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Duração</p><p className="text-sm font-black">{selecionado.duration}</p></div><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Aulas</p><p className="text-sm font-black">{concluidasSelecionado}/{totalAulas}</p></div><div className="rounded-2xl border border-white/10 bg-black p-3"><p className="text-[10px] text-zinc-500">Nível</p><p className="text-sm font-black">{levelLabel[selecionado.level]}</p></div></div>
            <div className="mt-4 grid gap-2"><button onClick={baixarMaterial} className="w-full rounded-2xl bg-white px-5 py-3 font-black text-black">Material</button><button onClick={baixarCertificado} className={`w-full rounded-2xl px-5 py-3 font-black ${porcentagemSelecionado === 100 ? "bg-emerald-400 text-black" : "border border-white/10 text-zinc-500"}`}>Certificado</button></div>
            <h3 className="mt-6 text-lg font-black text-white">Aulas</h3><div className="mt-3"><LessonPanel /></div>
          </aside>
