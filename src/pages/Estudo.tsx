...
export default function Estudo() {
  ...
  function abrirCurso(course: Course) {
    ...
    setSelecionado(course); setView("course"); window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  if (view === "course") {
    return (
      <div className="space-y-6">
        <button onClick={() => setView("catalog")} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-black text-zinc-300 transition hover:border-white/30 hover:bg-white/10">← Voltar para meus cursos</button>
        <main className="overflow-hidden rounded-[2.5rem] border border-white/10 bg-zinc-950 shadow-2xl shadow-black/50">
          <div className="relative overflow-hidden border-b border-white/10 bg-gradient-to-br from-white via-zinc-100 to-zinc-300 p-6 text-black md:p-10">
            <div className="relative mx-auto max-w-4xl text-center">
              ...
              <p className="mt-5 text-xs font-black uppercase tracking-[0.35em] text-zinc-600">{selecionado.category} • {levelLabel[selecionado.level]} • ID {selecionado.id}</p>
              ...
              <div className="mx-auto mt-5 h-3 max-w-xl overflow-hidden rounded-full bg-black/20"><div className="h-full rounded-full bg-black transition-all" style={{ width: `${porcentagemSelecionado}%` }} /></div>
          ...
          <div className="space-y-6 p-4 md:p-8">
            {porcentagemSelecionado === 100 && <section className="rounded-[2rem] border border-white/10 bg-white p-5 text-black md:p-6"><p className="text-4xl">🏆</p><h2 className="mt-3 text-2xl font-black">Curso concluído!</h2><p className="mt-2 text-sm font-semibold text-zinc-700">Você marcou todas as aulas como concluídas.</p></section>}
            <div className="grid gap-4 md:grid-cols-2"><div className="rounded-[2rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Resultado esperado</p><p className="mt-3 text-sm leading-7 text-zinc-300">{selecionado.outcome}</p></div><div className="rounded-[2rem] border border-white/10 bg-black p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Projeto final</p><p className="mt-3 text-sm leading-7 text-zinc-300">{selecionado.finalProject}</p></div></div>
            {selecionado.modules.map((modulo, moduloIndex) => <section key={modulo.title} className="overflow-hidden rounded-[2rem] border border-white/10 bg-black"><div className="border-b border-white/10 bg-zinc-950 p-5"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Módulo {moduloIndex + 1}</p><h2 className="mt-2 text-2xl font-black md:text-3xl">{modulo.title}</h2></div><div className="grid gap-4 p-4 md:p-5">{modulo.lessons.map((aula, aulaIndex) => { const key = lessonKey(selecionado.id, moduloIndex, aulaIndex); const done = progress[selecionado.id]?.includes(key) ?? false; return <article key={aula.title} className={`rounded-[1.5rem] border p-5 transition ${done ? "border-white/30 bg-white/[0.06]" : "border-white/10 bg-zinc-950"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase text-zinc-500">Aula {aulaIndex + 1}</p><h3 className="mt-2 text-2xl font-black">{aula.title}</h3></div><button onClick={() => toggleLesson(selecionado.id, key)} className={`rounded-full px-4 py-2 text-xs font-black transition ${done ? "bg-white text-black" : "border border-white/10 text-zinc-300 hover:bg-white/10"}`}>{done ? "Concluída ✓" : "Marcar como concluída"}</button></div><div className="mt-5 grid gap-4 lg:grid-cols-[1.25fr_0.75fr]"><div className="rounded-2xl border border-white/10 bg-black p-4"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Explicação</p><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-zinc-300">{aula.summary}</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs font-black uppercase tracking-widest text-zinc-500">Prática guiada</p><p className="mt-3 text-sm leading-7 text-zinc-200">{aula.practice}</p></div></div></article>; })}</div></section>)}
          </div>
        </main>
      </div>
    );
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-zinc-950 via-black to-zinc-950 p-6 md:p-8">
        <div className="relative">
          ...
          <h2 className="mt-5 text-4xl font-black md:text-5xl">Meus cursos liberados</h2>
          <p className="mt-2 max-w-2xl text-zinc-400">Cursos grátis, compras aprovadas e acessos liberados manualmente pelo ADM aparecem aqui.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-3xl border border-white/10 bg-black p-4"><p className="text-xs font-bold text-zinc-500">Liberados</p><p className="mt-1 text-2xl font-black">{liberadosCount}</p></div>
            <div className="rounded-3xl border border-white/10 bg-black p-4"><p className="text-xs font-bold text-zinc-500">Manuais ADM</p><p className="mt-1 text-2xl font-black">{manuaisCount}</p></div>
