...
export default function Pagamento() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <section className="relative overflow-hidden rounded-[2.6rem] border border-white/10 bg-white/[0.035] p-6 shadow-2xl shadow-black/40 backdrop-blur-xl md:p-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-4"><div><p className="text-sm font-black uppercase tracking-[0.18em] text-zinc-500">Valor exato para pagar</p><p className="text-5xl font-black text-emerald-200">R$ 29,90</p><p className="text-sm font-bold text-zinc-500 line-through">De R$ 49,90</p><p className="mt-1 text-xs font-black uppercase tracking-[0.16em] text-amber-200">Preço de lançamento honesto</p></div><button onClick={() => setProduto(produtos[0])} className="rounded-2xl bg-white px-6 py-4 font-black text-black shadow-lg shadow-amber-500/20 transition active:scale-95">Quero começar agora</button></div>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-black/50 p-5"><p className="text-6xl">⚡</p><h2 className="mt-4 text-3xl font-black">O que você vai aprender</h2><div className="mt-5 space-y-3">{["Entender energia elétrica sem confusão", "Calcular resistores com Lei de Ohm", "Montar circuitos simples em baixa tensão", "Usar multímetro no básico", "Ler esquemas simples", "Criar projeto final documentado"].map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm font-bold text-zinc-200">✓ {item}</div>)}</div></div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">{["Bônus: checklist de bancada segura", "Bônus: roteiro de projeto final", "Bônus: material para baixar"].map((item) => <div key={item} className="rounded-[2rem] border border-amber-400/20 bg-amber-500/10 p-5"><p className="text-3xl">🎁</p><h3 className="mt-3 text-xl font-black text-amber-100">{item}</h3><p className="mt-2 text-sm leading-6 text-amber-100/70">Incluído para deixar o aluno com caminho claro até o resultado final.</p></div>)}</section>

      <section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 md:p-6"><h2 className="text-3xl font-black">Módulos do curso principal</h2><div className="mt-5 grid gap-3 md:grid-cols-2">{modulos.map((modulo, index) => <div key={modulo.title} className="rounded-2xl border border-white/10 bg-black/35 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-zinc-500">Módulo {index + 1}</p><h3 className="mt-1 font-black text-white">{modulo.title}</h3><p className="mt-2 text-sm text-zinc-500">{modulo.lessons.length} aulas práticas</p></div>)}</div></section>
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_0.8fr]">
        <div className="rounded-[2rem] border border-white/10 bg-black/50 p-6 backdrop-blur-xl">
          ...
          <div className="mt-5 rounded-[2rem] border border-emerald-400/20 bg-emerald-500/10 p-5">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-200">Pague exatamente</p>
            <p className="mt-2 text-5xl font-black text-white">{produto.preco}</p>
            <p className="mt-2 text-sm font-bold text-emerald-100/80">Produto: {produto.nome}</p>
            <button onClick={copiarValor} className="mt-4 rounded-2xl bg-white px-5 py-3 text-sm font-black text-black">Copiar valor</button>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-500/10 p-4 text-sm text-amber-100"><p className="font-black">Obrigatório no WhatsApp</p><p className="mt-1">Envie o comprovante com: curso escolhido, ID do curso, valor pago, seu ID de usuário e ID da compra.</p><p className="mt-3 break-all text-xs">Seu ID: <span className="font-black">{userId || "entre na conta para aparecer"}</span></p><button onClick={copiarUserId} className="mt-3 rounded-xl bg-white px-4 py-2 text-xs font-black text-black">Copiar meu ID</button></div>
          ...
          <button onClick={registrarCompra} disabled={loading} className="mt-6 w-full rounded-2xl bg-white px-5 py-5 text-center text-lg font-black text-black disabled:opacity-60">{loading ? "Registrando..." : compraId ? "Registrar outra compra" : "Registrar compra"}</button>
