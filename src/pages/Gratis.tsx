...
function cleanTopic(input: string) { return input.trim(); }
...
function titleByIntent(intent: Intent, prompt: string) { if (intent === "checklist") return `Checklist pronto`; if (intent === "estrutura") return `Estrutura pronta`; if (intent === "ideia") return `Ideias prontas`; if (intent === "mensagem") return `Mensagem pronta`; return `Rascunho pronto`; }
function fallbackResult(rawInput: string, tone: Tone = "simples"): SmartResult { const input = rawInput.trim() || "mensagem rápida"; const intent = detectIntent(input); const topic = cleanTopic(input); if (intent === "mensagem") return { intent, title: "Mensagem pronta", text: `Oi! Tudo bem? Passando para falar sobre ${topic}.\n\nQueria resolver isso de forma simples e clara. Me responde quando puder?`, suggestions: ["mensagem pedindo desculpa", "mensagem cobrando dívida", "mensagem para professor"] }; if (intent === "checklist") return { intent, title: "Checklist pronto", text: `Checklist: ${topic}\n\n□ Separar o que é obrigatório\n□ Definir a primeira ação\n□ Conferir prazo ou limite\n□ Finalizar a parte principal\n□ Revisar antes de enviar`, suggestions: ["checklist de viagem", "checklist de estudo", "checklist de compras"] }; return { intent, title: titleByIntent(intent, topic), text: `Rascunho rápido: ${topic}\n\n1. Objetivo principal\n2. Informações importantes\n3. Próxima ação\n\nTexto inicial:\nPreciso resolver ${topic} de forma simples, direta e fácil de entender.`, suggestions: ["mensagem rápida", "checklist simples", "estrutura curta"] }; }
function getInitialPrompt() { try { const saved = localStorage.getItem("aprendaja_quick_prompt"); return saved || ""; } catch { return ""; } }
function saveHistory(prompt: string, result: SmartResult) { try { const saved = localStorage.getItem("aprendaja_history"); const current = saved ? JSON.parse(saved) : []; const next = [{ prompt, title: result.title, text: result.text, date: Date.now() }, ...current].filter((item, index, array) => array.findIndex((entry) => entry.prompt === item.prompt) === index).slice(0, 3); localStorage.setItem("aprendaja_history", JSON.stringify(next)); return next; } catch { return []; } }
...
export default function Gratis() {
  ...
  async function generateFromPrompt(value?: string, nextTone = tone) {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("smart-generate", { body: { prompt, tone: nextTone, mode: intent } });
      if (error) throw error;
      const text = String(data?.text || "").trim();
      if (!text) throw new Error("Resposta vazia");
      const nextResult = { intent, title: titleByIntent(intent, prompt), text, suggestions: quickExamples.filter((item) => item !== prompt).slice(0, 3) };
      setResult(nextResult);
      setHistory(saveHistory(prompt, nextResult));
    } catch (error) {
      const nextResult = fallbackResult(prompt, nextTone);
      setResult(nextResult);
      setHistory(saveHistory(prompt, nextResult));
      setToast("IA indisponível. Usei modo rápido por enquanto.");
    } finally {
