...
function ownedAvatars() {
  return new Set((localStorage.getItem(`${storage}:ownedAvatars`) || "default").split(",").filter(Boolean));
}

function saveOwned(set: Set<string>) {
  localStorage.setItem(`${storage}:ownedAvatars`, Array.from(set).join(","));
}

function NavButton({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`rounded-2xl px-4 py-3 text-sm font-black transition active:scale-[.98] ${active ? "bg-yellow-300 text-black shadow-[0_0_26px_rgba(250,204,21,.45)]" : "bg-white/[0.06] text-white/75 hover:bg-white/10"}`}>
      {children}
    </button>
  );
}
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,.06)]">
...
export default function GameApp() {
  ...
  function saveName() {
    const clean = playerName.trim().slice(0, 18) || "Jogador THK";
    setPlayerName(clean);
    writeText("playerName", clean);
    setLobbyMessage("Nome atualizado.");
    forceRefresh();
  }

  function copyRoomCode() {
    navigator.clipboard?.writeText(roomCode);
    setLobbyMessage("Código da sala copiado.");
  }

  function claimDaily() {
    const result = claimDailyReward();
    setDaily(getDailyRewardState());
    setLobbyMessage(result.claimed ? `Recompensa diária coletada: +${result.reward} moedas.` : "Você já coletou a recompensa de hoje.");
    forceRefresh();
  }

  function buyOrEquip(id: string, price: number) {
    const currentOwned = ownedAvatars();

    if (currentOwned.has(id)) {
      writeText("equippedAvatar", id);
      setShopMessage("Avatar equipado.");
      forceRefresh();
      return;
    }
  ...
  if (screen === "play") {
    return <ClashCardsV2 onBack={() => setScreen("home")} />;
  }

  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_50%_10%,rgba(124,58,237,.35),transparent_24rem),linear-gradient(180deg,#050008,#000)] px-4 py-5 text-white sm:px-6 lg:px-8">
      <section className="mx-auto max-w-7xl">
        ...
        <nav className="mb-4 flex gap-2 overflow-x-auto pb-2">
          <NavButton active={screen === "home"} onClick={() => setScreen("home")}>Lobby</NavButton>
          <NavButton active={screen === "modes"} onClick={() => setScreen("modes")}>Jogos</NavButton>
        ...
        {screen === "home" && (
          <LobbyHome
            ...
            onLobbyMessage={setLobbyMessage}
          />
        )}

        {screen === "modes" && (
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {gameIdentity.modes.map((mode, index) => (
              <button key={mode.id} onClick={() => index === 0 ? setScreen("play") : setSelectedMode(mode)} className={`rounded-[2rem] border p-6 text-left transition active:scale-[.99] ${index === 0 ? "border-violet-300/30 bg-violet-500/10 shadow-[0_0_55px_rgba(139,92,246,.14)]" : "border-white/10 bg-white/[0.045] opacity-85"}`}>
                <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-zinc-200">{mode.status}</span>
                <h2 className="mt-4 text-3xl font-black">{mode.title}</h2>
          ...
          <section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {avatarItems.map((item) => {
                ...
                return (
                  <button key={item.id} onClick={() => buyOrEquip(item.id, item.price)} className={`rounded-[2rem] border p-4 text-left transition active:scale-[.99] ${isEquipped ? "border-yellow-300 bg-yellow-300/15" : "border-white/10 bg-black/35 hover:bg-white/[0.06]"}`}>
                    <div className={`grid h-24 place-items-center rounded-[1.5rem] bg-gradient-to-br ${item.gradient} text-5xl shadow-[0_20px_55px_rgba(0,0,0,.42)]`}>{item.emoji}</div>
