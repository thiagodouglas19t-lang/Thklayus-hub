...
function SmallButton({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return <button onClick={onClick} className="rounded-2xl border border-white/10 bg-black/50 px-3 py-3 text-[10px] font-black text-white backdrop-blur transition active:scale-[.97]">{children}</button>;
}
...
export default function LobbyHome({ playerName, equipped, stats, daily, roomCode, lobbyMessage, selectedMode, onPlay, onClaimDaily, onCopyRoomCode, onSelectMode, onLobbyMessage }: LobbyHomeProps) {
  ...
  return (
    <section className="relative mx-auto max-w-[430px] overflow-hidden rounded-[1.8rem] border border-white/10 bg-[radial-gradient(circle_at_50%_5%,rgba(14,165,233,.22),transparent_14rem),radial-gradient(circle_at_50%_32%,rgba(168,85,247,.42),transparent_14rem),linear-gradient(180deg,#07111c,#050008_44%,#000)] p-3 shadow-[0_24px_90px_rgba(0,0,0,.78)] sm:max-w-[480px]">
      ...
      <div className="relative z-10 space-y-3">
        <section className="relative min-h-[440px] overflow-hidden rounded-[1.65rem] border border-white/10 bg-[radial-gradient(circle_at_50%_36%,rgba(34,211,238,.2),transparent_12rem),radial-gradient(circle_at_50%_38%,rgba(168,85,247,.4),transparent_13rem),linear-gradient(180deg,rgba(255,255,255,.055),rgba(0,0,0,.54))] p-3">
          <div className="absolute left-3 top-3 z-30 flex w-20 flex-col gap-2">
          ...
          <div className="absolute right-3 top-3 z-30 flex w-20 flex-col gap-2">
          ...
          <div className="absolute left-1/2 top-4 z-20 -translate-x-1/2 text-center">
            ...
            <h2 className="text-3xl font-black leading-none">Arena</h2>
            <p className="mt-1 text-[10px] uppercase tracking-[0.2em] text-zinc-400">{selectedMode.title}</p>
          </div>

          <div className="absolute left-1/2 top-[48%] h-20 w-64 -translate-x-1/2 rounded-[50%] border border-cyan-200/20 bg-black/65 shadow-[0_0_70px_rgba(34,211,238,.2)]" />
          <div className="absolute left-1/2 top-[43%] z-20 -translate-x-1/2 -translate-y-1/2">
            <div className={`relative grid h-60 w-40 place-items-center rounded-[2rem] bg-gradient-to-br ${avatarGradient} text-7xl shadow-[0_38px_90px_rgba(0,0,0,.78),0_0_88px_rgba(139,92,246,.36)] ring-2 ring-white/15`}>
          ...
          <div className="absolute bottom-[86px] left-3 right-3 z-30 grid grid-cols-4 gap-2">
            {partySlots.map((member, index) => (
              <button key={`${member.name}-${index}`} onClick={() => index > 0 && setOnlineOpen(true)} className={`rounded-2xl border p-2 text-center backdrop-blur transition active:scale-[.97] ${index === 0 ? "border-yellow-300/45 bg-yellow-300/12" : member.ready ? "border-cyan-300/25 bg-black/45" : "border-dashed border-white/15 bg-black/35"}`}>
                <div className={`mx-auto grid h-9 w-9 place-items-center rounded-xl ${index === 0 ? `bg-gradient-to-br ${avatarGradient}` : "bg-white/8"} text-lg`}>{index === 0 ? avatarEmoji : member.emoji}</div>
                <strong className="mt-1 block truncate text-[9px]">{index === 0 ? "Você" : member.name}</strong>
              </button>
            ))}
          </div>

          <div className="absolute bottom-3 left-3 right-3 z-40 grid grid-cols-[1fr_78px] gap-2">
            <button onClick={startMatchmaking} className="rounded-[1.2rem] bg-gradient-to-b from-yellow-200 to-yellow-500 px-5 py-4 text-2xl font-black text-black shadow-[0_7px_0_#8a4d00,0_24px_55px_rgba(0,0,0,.58)] transition hover:brightness-110 active:translate-y-1 active:shadow-[0_3px_0_#8a4d00]">START</button>
        ...
        <CardPanel>
          <div className="grid grid-cols-2 gap-2">
            {gameIdentity.modes.slice(0, 4).map((mode) => <button key={mode.id} onClick={() => { playUiSound("click"); onSelectMode(mode); }} className={`rounded-2xl border px-3 py-3 text-left transition active:scale-[.98] ${selectedMode.id === mode.id ? "border-yellow-300 bg-yellow-300/15" : "border-white/10 bg-white/[0.045]"}`}><strong className="block text-sm">{mode.title}</strong><p className="text-[10px] text-zinc-500">{mode.status}</p></button>)}
          </div>
        </CardPanel>

        <div className="grid grid-cols-2 gap-2">
          <CardPanel className="border-yellow-300/15 bg-yellow-300/[0.07]"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-yellow-300">Daily</p><strong className="mt-1 block text-2xl">🔥 {daily.streak}</strong><button onClick={() => { playUiSound("confirm"); onClaimDaily(); }} className={`mt-2 w-full rounded-xl px-3 py-2 text-xs font-black ${daily.canClaim ? "bg-yellow-300 text-black" : "bg-white/10 text-white/45"}`}>{daily.canClaim ? "Coletar" : "OK"}</button></CardPanel>
          <CardPanel><p className="text-[10px] font-black uppercase tracking-[0.22em] text-yellow-300">Status</p><p className="mt-2 min-h-12 text-xs text-zinc-300">{lobbyMessage}</p></CardPanel>
        </div>
      </div>
